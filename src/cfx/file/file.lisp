...
(defmethod mon-average-value ((mon mnas-ansys/cfx/file/mon:<mon>))
  (math/stat:average-value
   (mnas-ansys/cfx/file/mon:<mon>-data mon)))

(defmethod mon ((key string) (res <res>))
  (gethash key (<res>-mon res)))


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defun open-last-cfx-file (res-prj-dir)
  "@b(Описание:) функция @b(open-last-cfx-file) возвращает объект класса
<res>, который будет найден последним в каталоге @b(res-prj-dir).

 @b(Переменые:)
@begin(list)
 @item(res-prj-dir - строка, представляющая каталог с res-файлами.)
@end(list)
"
  (let* ((res-file-names
           (directory (concatenate 'string res-prj-dir "/*.res")))
         (res (open-cfx-file
               (first (last res-file-names)))))
  res))
